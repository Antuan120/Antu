...
public class ResulltadosDAO {
    ...
    public String[] seleccionesMasGoles() {
        ...
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT seleccion FROM (SELECT local AS seleccion, SUM(goles_local) AS goles FROM a_penaranda12.partidos GROUP BY local union SELECT visitante AS seleccion, SUM(goles_visitante) AS goles FROM a_penaranda12.partidos GROUP BY visitante) t WHERE goles = (SELECT MAX(goles) FROM ( SELECT SUM(goles_local) AS goles FROM a_penaranda12.partidos GROUP BY local union SELECT SUM(goles_visitante) AS goles FROM a_penaranda12.partidos GROUP BY visitante) s)");
            while (result.next()) {
